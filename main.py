...
Configuração:
  ...
  Use scripts/get_cookies.py para extrair token do navegador.
    ...
    group.add_argument("--status", action="store_true", help="Mostrar status do agendamento")

    parser.add_argument(
        "--hour", type=int, default=None, help="Hora para agendamento (0-23, padrão: 12)"
    )

    parser.add_argument(
        "--minute", type=int, default=None, help="Minuto para agendamento (0-59, padrão: 0)"
    )

    parser.add_argument(
        "--version", "-v", action="version", version=f"Epic Games Claimer v{_get_version()}"
    )
...
def main() -> int:
    ...
    try:
        if args.status:
            ...
            if games:
                logger.info(f"{len(games)} jogo(s) disponível(is) para resgate")
                for game in games:
                    logger.info(f"  {game['title']}")
            else:
                logger.info("Nenhum jogo novo disponível")
