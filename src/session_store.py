...
class Session:
    ...
    @classmethod
    def from_eg1_token(cls, eg1_token: str) -> "Session | None":
        """
...
class SessionStore:
    ...
    def save(self, session: Session) -> bool:
        ...
        try:
            ...
            self._logger.debug(
                "Session saved", path=str(self.session_file), account=session.display_name
            )
            return True
    ...
    def refresh_from_chrome(self, config: "Config | None" = None) -> Session | None:
        """
        ...
        try:
            ...
            self._logger.info("Extraindo cookies do Chrome...")
            ...
            if self.save(session):
                self._logger.success(f"Sessão atualizada do Chrome: {session.display_name}")
