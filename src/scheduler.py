...
class Scheduler:
    ...
    def _wait_until(self, target: datetime) -> None:
        ...
        while self._running and datetime.now() < target:
            # Sleep in 60-second intervals to allow for graceful shutdown
    ...
    def check_schedule_status(self) -> None:
        ...
        print("=" * 50)
        print(f"Horário agendado: {self.config.schedule_hour:02d}:{self.config.schedule_minute:02d}")
        print(f"Próxima execução: {next_run.strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Tempo restante:   {self.format_duration(wait_time)}")
        print("=" * 50 + "\n")
